...
is the definition of a _constructor_ in Go—would
...
but only _re-assigned_ in the second.
...
variable. Such a _type switch_ uses the syntax of a type
...
In Go, `Write`
can return a count _and_ an error: “Yes, you wrote some
...
_deferred_ function) to be run immediately before the function
...
the function is a method) are evaluated when the _defer_
executes, not when the _call_ executes. Besides avoiding worries
...
in some other languages it does not _initialize_ the memory,
it only _zeros_ it.
That is,
...
using a _composite literal_, which is
...
However, by labeling the elements explicitly as _field_`:` _value_
...
Back to allocation.
The built-in function `make(T,` _args_`)` serves
a purpose different from `new(T)`.
It creates slices, maps, and channels only, and it returns an _initialized_
(not _zeroed_)
...
- In particular, if you pass an array to a function, it
  will receive a _copy_ of the array, not a pointer to it.
...
`buf`, _slice_ (here used as a verb) the buffer.
...
itself. The _capacity_ of a slice, accessible by the built-in
...
values of one type (the _key_) with values of another type
(the _element_ or _value_).
...
you can use the [blank identifier](https://go.dev/doc/effective_go#blank) (`_`)
...
Moreover, that format can print _any_ value, even arrays, slices, structs, and
...
(If you need to print _values_ of type `T` as well as pointers to `T`,
the receiver for `String` must be of value type; this example used a pointer because
that's more efficient and idiomatic for struct types.
See the section below on [pointers vs. value receivers](https://go.dev/doc/effective_go#pointers_vs_values) for more information.)
...
where _T_ is a placeholder for any given type. You can't
...
_pointer_ to a `ByteSlice` as its receiver, so the
...
object: if something can do _this_, then it can be used
_here_. We've seen a couple of simple examples already;
...
[Type switches](https://go.dev/doc/effective_go#type_switch) are a form of conversion: they take an interface and, for
...
A one-case type switch would do, but so would a _type assertion_.
...
[`for``range` loops](https://go.dev/doc/effective_go#for)
and [maps](https://go.dev/doc/effective_go#maps).
...
As we saw in the discussion of [interfaces](https://go.dev/doc/effective_go#interfaces_and_types) above,
...
The encoder checks this property at run time with a [type assertion](https://go.dev/doc/effective_go#interface_conversions) like:
...
implementation by _embedding_ types within a struct or
interface.
...
what a `Reader` does _and_ what a `Writer`
...
They're called _goroutines_ because the existing

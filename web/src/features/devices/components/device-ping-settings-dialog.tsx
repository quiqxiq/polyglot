...
export function DevicePingSettingsDialog({
  ...
  const { data: configData, isLoading: isConfigLoading } =
    useDevicePingConfigQuery(device.id, open)
  const updateMutation = useUpdateDevicePingConfigMutation()

  const [enabled, setEnabled] = useState<boolean>(false)
  const [target, setTarget] = useState<string>('8.8.8.8')
  const [retentionDays, setRetentionDays] = useState<number>(7)

  useEffect(() => {
    if (configData?.config) {
      setEnabled(configData.config.enabled)
      setTarget(configData.config.target || '8.8.8.8')
      setRetentionDays(configData.config.retentionDays || 7)
    } else {
      setEnabled(device.pingEnabled)
      setTarget(device.pingTarget || '8.8.8.8')
      setRetentionDays(device.pingRetentionDays || 7)
    }
  }, [configData, device])
  ...
  const handleSave = async () => {
    try {
      ...
      console.error('Failed to update ping config', err)
    }
  }
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="flex items-center justify-between rounded-lg border p-3 bg-muted/30">
              ...
              {timescaledbAvailable ? (
                <Badge variant="outline" className="bg-emerald-500/10 text-emerald-600 border-emerald-200 text-[11px] gap-1">

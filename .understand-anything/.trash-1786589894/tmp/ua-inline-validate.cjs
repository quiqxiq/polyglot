...
try {
  ...
  graph.edges.forEach((e, i) => {
    if (!e.source || !nodeIds.has(e.source)) issues.push(`Edge[${i}] source '${e.source}' not found`);
    if (!e.target || !nodeIds.has(e.target)) issues.push(`Edge[${i}] target '${e.target}' not found`);
  });
  ...
  graph.layers.forEach(layer => {
    ...
    if (!Array.isArray(layer.nodeIds)) issues.push(`Layer '${layer.id || '?'}' missing nodeIds`);

...
// RBACConnectHandler implements ConnectRPC procedures for RBAC policy management.
type RBACConnectHandler struct {
	enforcer *auth.CasbinEnforcer
}

// NewRBACConnectHandler returns a new RBACConnectHandler.
func NewRBACConnectHandler(enforcer *auth.CasbinEnforcer) *RBACConnectHandler {
	return &RBACConnectHandler{enforcer: enforcer}
}

// ListPolicies returns all active RBAC policies.
func (h *RBACConnectHandler) ListPolicies(ctx context.Context, req *connect.Request[devicepb.ListPoliciesRequest]) (*connect.Response[devicepb.ListPoliciesResponse], error) {
	...
	return connect.NewResponse(&devicepb.ListPoliciesResponse{
		Policies: mapper.PolicyRowsToProto(rawPolicies),
	}), nil
}
...
// RemovePolicy removes an existing policy rule.
...
// ListRoleAssignments returns all role-user assignments.
func (h *RBACConnectHandler) ListRoleAssignments(ctx context.Context, req *connect.Request[devicepb.ListRoleAssignmentsRequest]) (*connect.Response[devicepb.ListRoleAssignmentsResponse], error) {
	...
	groupingPolicies, err := h.enforcer.GetGroupingPolicies()
	if err != nil {
		return nil, response.ToConnectError(err)
	}

	assignments := make([]*devicepb.RoleAssignment, 0, len(groupingPolicies))
...
// UnassignRole revokes a role from a user.
...
// NewRBACServiceHandler creates the Connect http.Handler and registers procedures.
func NewRBACServiceHandler(enforcer *auth.CasbinEnforcer) (string, http.Handler) {
	...
	return "/polyglot.v1.RBACService/", mux
}

...
type RouterAccountManager interface {
	...
	// username (best-effort, dipakai pada alur terminate) — F6-3.
	CleanupIsolationAddressList(ctx context.Context, deviceID, addressList, username string) error
	...
	// DeleteIsolationInfrastructure menghapus profil isolir (+ opsional rule
	// firewall/walled-garden) dari router — F6-6.
	DeleteIsolationInfrastructure(ctx context.Context, deviceID string, cfg domainDevice.IsolationConfig, removeFirewallRules bool) error
